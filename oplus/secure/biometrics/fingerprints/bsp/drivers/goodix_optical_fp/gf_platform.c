        ...
        bool enable)
{
    ...
    if (enable) {
        ...
        if (regulator_count_voltages(vreg) > 0) {
            ...
            if (rc) {
                pr_debug("Unable to set voltage on %s, %d\n", name, rc);
            }
        }
        rc = regulator_set_load(vreg, pwr_info->vreg_config.ua_load);
        if (rc < 0) {
            pr_debug("Unable to set current on %s, %d\n", name, rc);
        }
        rc = regulator_enable(vreg);
        if (rc) {
            pr_debug("error enabling %s: %d\n", name, rc);
            regulator_put(vreg);
            vreg = NULL;
        }
        pwr_info->vreg = vreg;
...
int gf_parse_pwr_list(struct gf_dev* gf_dev) {
    ...
    for_each_available_child_of_node(np, child) {
        ...
        if (ret) {
            pr_debug("failed to request %s, ret = %d\n", FP_POWER_NODE, ret);
            goto exit;
        }
        ...
        case FP_POWER_MODE_LDO:
            ...
            if (ret) {
                pr_debug("the param %s is not found !\n", FP_POWER_NAME_NODE);
                ret = -FP_ERROR_GENERAL;
                goto exit;
            }
            ...
            if(ldo_param_amount != LDO_PARAM_AMOUNT) {
                pr_debug("failed to request size %s\n", node_name);
                ret = -FP_ERROR_GENERAL;
                goto exit;
            }
            ...
            if (ret) {
                pr_debug("failed to request %s(%d), rc = %u\n", node_name, LDO_VMAX_INDEX, ret);
                goto exit;
            }
        ...
        case FP_POWER_MODE_GPIO:
            ...
            if (gf_dev->pwr_list[child_node_index].pwr_gpio < 0) {
                pr_debug("falied to get goodix_pwr gpio!\n");
                ret = -FP_ERROR_GENERAL;
                goto exit;
            }
...
int gf_parse_ftm_poweroff_flag(struct gf_dev* gf_dev) {
    ...
    pr_debug("gf_dev->ftm_poweroff_flag = %d\n", gf_dev->ftm_poweroff_flag);
...
int gf_parse_notify_tpinfo_flag(struct gf_dev* gf_dev) {
    ...
    pr_debug("gf_dev->notify_tpinfo_flag = %d\n", gf_dev->notify_tpinfo_flag);
...
int gf_parse_dts(struct gf_dev* gf_dev)
{
    ...
    if (rc) {
        pr_debug("failed to parse power list, rc = %d\n", rc);
        goto err_pwr;
    }
...
int gf_power_on(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        if (rc) {
            pr_debug("---- power on failed with mode = %d, index = %d, rc = %d ----\n",
...
int gf_power_off(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        if (rc) {
            pr_debug("---- power off failed with mode = %d, index = %d, rc = %d ----\n",
                    gf_dev->pwr_list[index].pwr_type, index, rc);
            break;
        } else {
