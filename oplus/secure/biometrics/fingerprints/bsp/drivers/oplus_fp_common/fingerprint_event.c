...
int wait_fingerprint_event(void *data, unsigned int size,
    ...
    if ((ret = wait_event_interruptible(fp_queue, g_reporte_condition == 1)) !=
        0) {
      pr_debug("fp driver wait event fail, %d", ret);
    }
    if (msg != NULL) {
...
{
    pr_debug("fp_event_node_read enter");
    if (file == NULL || count != sizeof(g_fp_dirver_event_type)) {
        return -1;
    }
    pr_debug("fp_event_node_read,  %d", g_fp_dirver_event_type);
...
{
    pr_debug("set_fp_driver_event_type, %d", type);
...
int send_fingerprint_message(int module, int event, void *data,
    ...
    default:
        ...
        need_report = 1;
        pr_debug("unknow module, ignored");
        break;
    }
    if (need_report)
