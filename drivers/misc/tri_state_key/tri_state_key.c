...
static void tri_key_dev_work(struct work_struct *work)
{
	...
	if (key[0] && key[1] && key[2]) {
		TRI_KEY_LOG("key[0]=%d,key[1]=%d,key[2]=%d***pre_key0=%d, pre_key1=%d, pre_key2=%d\n",
		key[0], key[1], key[2], pre_key0, pre_key1, pre_key2);
		if (pre_key0 == 1 && pre_key1 == 1 && pre_key2 == 0)
			return;
		if (pre_key0 && pre_key1 && pre_key2) {
			trikey_data->state = 3;
			TRI_KEY_LOG("Debounce timeout: Set the tri key state to 0,1,1\n");
			...
			mod_timer(&trikey_data->s_timer, jiffies + HZ/5);
			pre_key0 = key[0];
			pre_key1 = key[1];
			pre_key2 = key[2];
			TRI_KEY_LOG("Wait for debounce timeout\n");
		}
		return;
	} else {
		if (!del_timer(&trikey_data->s_timer))
			TRI_KEY_LOG("timer already expired for floating state\n");
	}
	if (!key[0] && !key[1] && !key[2])
		return;
	if (!key[0] && !key[1] && key[2])
...
#else
static inline int
dev_get_devtree_pdata(struct device *dev)
{
	TRI_KEY_LOG("inline function\n");
	return 0;
}
#endif
...
static int tri_key_dev_probe(struct platform_device *pdev)
{
	...
	/*init_timer(&trikey_data->s_timer);*/
	__init_timer(&trikey_data->s_timer, timer_handle, 0);
